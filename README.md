...
| 数据 | 数量 | 说明 |
|------|------|------|
| 用户 | 2 | testuser1 (18 分)、testuser2 (9 分) |
| 比赛 | 16 | 世界杯 8 场 (真实国家队：巴西/阿根廷/法国/德国等) + 苏超 8 场 (江苏省地级市代表队：南京队/苏州队/无锡队等)，8 场已结束 (FINISHED)、8 场未开始 (SCHEDULED) |
| 预测 | 32 | 16 条已结算 (部分猜中 3 分、部分 2 分/1 分、部分 0 分) + 16 条未结算 |
| 评论 | 6 | 巴西 vs 德国 3 条树形回复 + 南京队 vs 苏州队 3 条树形回复 |

比赛日期为 2026 年 6-7 月，已结束比赛填入合理比分 (如 2-1、0-0、3-1 等)。
...
1. 希望能增加更多实战部署环节，如 Docker 部署、CI/CD 流水线的动手练习，目前这部分以理论为主。
2. 前端测试覆盖率可以加强，课程中侧重了后端测试，建议补充 React 组件测试和 E2E 测试的实践。
3. 建议在课程中引入 WebSocket 实时通信的案例，例如比赛比分实时推送，让项目更贴近真实场景。
4. 希望提供更多关于数据库性能优化 (索引、查询计划分析) 的深入讲解和练习。
